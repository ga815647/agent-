# Chat Dev Repo-Centered Migration Plan
...
1. Make `chat-dev/` available on production `main` at its final canonical repo path without yet changing existing Project Instructions.
2. Build/identify the immutable accepted release snapshot.
3. Activate stable `chat-dev/BOOTSTRAP.md` to select that release.
4. Update Project Instructions to the minimal repo bootstrap shim in a controlled staged manner.
5. Keep Notion `Current/BRAIN/W` available as compatibility/rollback sources during the rollback window.
6. After stable use, demote those Notion pages to pointer-only/index roles where appropriate.

Do not move project operational/research state as part of activation.
...
| Notion `Chat Dev｜Current` | repo `chat-dev/BOOTSTRAP.md` + `ARCHITECTURE.md` | compatibility pointer/dashboard entry |
| Notion `Chat Dev｜BRAIN` | repo `chat-dev/BRAIN.md` + detailed BRAIN contract | compatibility pointer |
| Notion `Chat Dev｜W` | repo `chat-dev/W.md` + architecture Worker section | compatibility pointer |
| Notion Handoff Authoring Guide | repo `chat-dev/HANDOFF-AUTHORING.md` | pointer/archive after rollback window |
| Notion Project Instructions Sample | repo `chat-dev/PROJECT-INSTRUCTIONS-SHIM.md` / adoption guide | pointer/archive as appropriate |
| Project Profiles | project-local durable source | remain project-local |
| `chat-dev-control-plane-v0/ARCHITECTURE.md` | repo `chat-dev/ARCHITECTURE.md` | compatibility pointer or historical baseline |
| `BRAIN-AUTO-PILOT.md` | detailed BRAIN implementation contract | retain if useful; no competing current-entry authority |
